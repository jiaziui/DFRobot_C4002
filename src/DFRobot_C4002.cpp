...
bool DFRobot_C4002::setReportPeriod(uint8_t period)//范围0-255.单位100ms
{
  uint8_t sendData[10];
  uint16_t dataLen = 0;
  uint16_t temp = 5;
...
sRecvPack_t DFRobot_C4002::recvPack()
{
  ...
  if( recvLen == 8 && pdata[0] == FRAME_HEADER1 && pdata[1] == FRAME_HEADER2
      && pdata[2] == FRAME_HEADER3 && pdata[3] == FRAME_HEADER4){
