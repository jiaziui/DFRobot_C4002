...
CMD_FACTORY_RESET                =  0x02             # factory reset command
...
class DetectResultStruct(Structure):
  _fields_ = [("target_status", c_uint8),
              ("light", c_uint16),
...
class RecvPack(Structure):
  _fields_ = [
      ...
      ("data", c_uint8 * 50),
      ("pack_type", c_uint8),
      ("result_pon_code",c_uint8)]

class ResData(Structure):
  _fields_ = [
      ("cmd", c_uint8),
      ("resp_code", c_uint8),
      ("dect_result", DetectResultStruct),
      ("calibration_down", c_uint16)]
...
class DFRobot_C4002(object):
  ...
  def recv_pack(self):
    ...
    if( recv_pack[0] == FRAME_HEADER1 and
        recv_pack[1] == FRAME_HEADER2 and
        recv_pack[2] == FRAME_HEADER3 and
        recv_pack[3] == FRAME_HEADER4):
      ...
      if(len(recv_pack) == packlen):
        if self.check_sum(recv_pack,packlen):
          ...
          if not (recv_dat.pack_type == FRAME_TYPE_WRITE_RESPOND or
                  recv_dat.pack_type == FRAME_TYPE_READ_RESPOND or
                  recv_dat.pack_type == FRAME_TYPE_NOTIFICATION):
